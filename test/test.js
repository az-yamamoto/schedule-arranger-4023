...
describe('/schedules/:scheduleId/users/:userId/comments', () => {
  ...
  test('コメントが更新できる', async () => {
    ...
    const { formToken, cookieToken } = await getCSRFTokens();
    const res = await request(app).post('/schedules').set('Cookie', `csrfToken=${cookieToken}`).send({
      _csrf: formToken,
...
describe('/schedules/:scheduleId/delete', () => {
  ...
  test('予定に関連する全ての情報が削除できる', async () => {
    ...
    // 削除
    await request(app).post(`/schedules/${scheduleId}/delete`).set('Cookie', `csrfToken=${cookieToken}`).send({ _csrf: formToken });
